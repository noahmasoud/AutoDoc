...
from sqlalchemy import create_engine, inspect, select, text, event, func
...
class TestSchemaCreation:
    ...
    def test_run_table_columns(self, test_engine):
        ...
        required_columns = {
            "id", "repo", "branch", "commit_sha",
            "started_at", "completed_at", "status", "correlation_id"
        }
    ...
    def test_change_table_columns(self, test_engine):
        ...
        required_columns = {
            "id", "run_id", "file_path", "symbol",
            "change_type", "signature_before", "signature_after"
        }
        assert required_columns.issubset(columns)
