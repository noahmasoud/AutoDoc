"""
JavaScript Analyzer Service for CI/CD Integration

This module processes changed JavaScript files in CI/CD runs,
parses them using the JavaScript AST parser, and logs results
to the run report.
"""
...
from services.javascript_parser import ParseError, JavaScriptParser
...
class JavaScriptAnalyzer:
    """
    Analyzer for processing JavaScript files in CI/CD runs.

    Detects changed .js files, parses them to extract symbols,
    ...
    def analyze_changed_files(
        ...
        """
        Analyze changed JavaScript files and extract public symbols.
        ...
        """
        logger.info(
            f"[Run: {run_id}] Starting JavaScript file analysis",
            extra={"run_id": run_id, "files_count": len(changed_files)},
        )

        # Filter to only JavaScript files
        js_files = [f for f in changed_files if self._is_javascript_file(f)]

        if not js_files:
            logger.info(
                f"[Run: {run_id}] No JavaScript files to analyze",
                extra={"run_id": run_id},
            )
            return {
                "files_processed": 0,
        ...
        logger.info(
            f"[Run: {run_id}] Found {len(js_files)} JavaScript files to analyze",
            extra={"run_id": run_id, "js_files": js_files},
        )
