...
class DatabaseSettings(BaseSettings):
    ...
    max_overflow: int = Field(default=10, description="Maximum database connection overflow")
    pool_timeout: int = Field(default=30, description="Database connection pool timeout in seconds")
    pool_recycle: int = Field(default=3600, description="Database connection recycle time in seconds")
...
class ConfluenceSettings(BaseSettings):
    ...
    url: Optional[str] = Field(default=None, description="Confluence base URL")
    username: Optional[str] = Field(default=None, description="Confluence username")
    token: Optional[str] = Field(default=None, description="Confluence API token")
    
    # Confluence settings
    space_key: Optional[str] = Field(default=None, description="Default Confluence space key")
...
class LoggingSettings(BaseSettings):
    ...
    max_size: int = Field(default=10485760, description="Maximum log file size in bytes")  # 10MB
...
class Settings(BaseSettings):
    ...
    # Python path
    pythonpath: Optional[str] = Field(default=None, description="Python path")
...
def validate_required_secrets() -> None:
    ...
    if settings.is_production:
        if not settings.confluence.is_configured:
            print("Warning: Confluence is not configured for production")
...
def print_config_summary() -> None:
    ...
    print("=" * 50)
    print(f"AutoDoc Configuration Summary")
    print("=" * 50)
    ...
    print(f"Redis: {settings.redis.url or 'Not configured'}")
    print(f"API Host: {settings.api.host}:{settings.api.port}")
    print(f"Confluence: {'Configured' if settings.confluence.is_configured else 'Not configured'}")
