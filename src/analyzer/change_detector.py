...
class SymbolChange:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'change_type': self.change_type.value, 'symbol_type': self.symbol_type.value, 'symbol_name': self.symbol_name, 'is_breaking': self.is_breaking, 'breaking_reasons': self.breaking_reasons, 'details': self.details}
...
class ChangeReport:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'file_path': self.file_path, 'old_version': self.old_version, 'new_version': self.new_version, 'added': [c.to_dict() for c in self.added], 'removed': [c.to_dict() for c in self.removed], 'modified': [c.to_dict() for c in self.modified], 'has_breaking_changes': self.has_breaking_changes, 'summary': {'total_changes': len(self.added) + len(self.removed) + len(self.modified), 'added_count': len(self.added), 'removed_count': len(self.removed), 'modified_count': len(self.modified), 'breaking_count': sum(1 for c in self.added + self.removed + self.modified if c.is_breaking)}}


class FunctionChangeDetector:
    def compare(self, old_func: Optional[FunctionInfo], new_func: Optional[FunctionInfo]) -> Optional[SymbolChange]:
        if old_func is None and new_func is not None:
            return SymbolChange(change_type=ChangeType.ADDED, symbol_type=SymbolType.METHOD if new_func.is_method else SymbolType.FUNCTION, symbol_name=new_func.name)
        if old_func is not None and new_func is None:
            return SymbolChange(change_type=ChangeType.REMOVED, symbol_type=SymbolType.METHOD if old_func.is_method else SymbolType.FUNCTION, symbol_name=old_func.name, is_breaking=old_func.is_public, breaking_reasons=['Public function removed'] if old_func.is_public else [])
        if old_func and new_func:
            return self._detect_modifications(old_func, new_func)
        return None

    def _detect_modifications(self, old_func: FunctionInfo, new_func: FunctionInfo) -> Optional[SymbolChange]:
        breaking_reasons = []
        details = {}
        ...
        if old_func.return_type and new_func.return_type and old_func.return_type != new_func.return_type:
            breaking_reasons.append(
        ...
        if not breaking_reasons and not details:
            return None
        return SymbolChange(change_type=ChangeType.MODIFIED, symbol_type=SymbolType.METHOD if new_func.is_method else SymbolType.FUNCTION, symbol_name=new_func.name, is_breaking=len(breaking_reasons) > 0 and new_func.is_public, breaking_reasons=breaking_reasons if new_func.is_public else [], details=details)
...
class ClassChangeDetector:
    ...
    def compare(self, old_class: Optional[ClassInfo], new_class: Optional[ClassInfo]) -> Optional[SymbolChange]:
        if old_class is None and new_class is not None:
            return SymbolChange(change_type=ChangeType.ADDED, symbol_type=SymbolType.CLASS, symbol_name=new_class.name)
        if old_class is not None and new_class is None:
            return SymbolChange(change_type=ChangeType.REMOVED, symbol_type=SymbolType.CLASS, symbol_name=old_class.name, is_breaking=old_class.is_public, breaking_reasons=['Public class removed'] if old_class.is_public else [])
        if old_class and new_class:
            return self._detect_modifications(old_class, new_class)
        return None

    def _detect_modifications(self, old_class: ClassInfo, new_class: ClassInfo) -> Optional[SymbolChange]:
        breaking_reasons = []
        details = {}
        if set(old_class.base_classes) != set(new_class.base_classes):
            breaking_reasons.append("Base classes changed")
            details['base_classes'] = {
                'old': old_class.base_classes, 'new': new_class.base_classes}
        ...
        if not breaking_reasons and not details:
            return None
        return SymbolChange(change_type=ChangeType.MODIFIED, symbol_type=SymbolType.CLASS, symbol_name=new_class.name, is_breaking=len(breaking_reasons) > 0 and new_class.is_public, breaking_reasons=breaking_reasons if new_class.is_public else [], details=details)
...
class ChangeDetector:
    ...
    def detect_changes(self, old_module: ModuleInfo, new_module: ModuleInfo, old_version: str = "old", new_version: str = "new") -> ChangeReport:
        report = ChangeReport(file_path=new_module.file_path,
                              old_version=old_version, new_version=new_version)
        self._compare_symbols(old_module.functions,
                              new_module.functions, self.func_detector, report)
        self._compare_symbols(old_module.classes,
                              new_module.classes, self.class_detector, report)
...
def detect_changes(old_module: ModuleInfo, new_module: ModuleInfo, old_version: str = "old", new_version: str = "new") -> ChangeReport:
